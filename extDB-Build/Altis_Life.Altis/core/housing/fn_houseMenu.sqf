...
if(_curTarget isKindOf "House_F" && playerSide == west OR playerSide == east) exitWith {
	if((nearestObject [[5477.9092,15002.27,29.29999],"Land_Research_house_V1_F"]) == _curTarget 
	OR (nearestObject [[15481.608,15675.839,4],"Land_Dome_Big_F"]) == _curTarget 
	OR (nearestObject [[15474.226,15725.228,4],"Land_Dome_Big_F"]) == _curTarget 
	OR (nearestObject [[15474.214,15698.04,0],"Land_Research_house_V1_F"]) == _curTarget 
	OR (nearestObject [[16640.793,15695.068,5.5576954],"Land_Cargo_Tower_V1_No5_F"]) == _curTarget 
	OR (nearestObject [[16677.453,15715.972,10.5],"Land_Cargo_Tower_V1_No5_F"]) == _curTarget
	OR (nearestObject [[16158.405,16963.377,0],"Land_Research_house_V1_F"]) == _curTarget
	OR (nearestObject [[16013.316,17043.652,0],"Land_Research_house_V1_F"]) == _curTarget
	OR (nearestObject [[16079.912,17088.65,0],"Land_Research_house_V1_F"]) == _curTarget
	OR (nearestObject [[16155.282,17058.164,0],"Land_Research_house_V1_F"]) == _curTarget
	OR (nearestObject [[19544.648,15339.452,0],"Land_Research_house_V1_F"]) == _curTarget
		...
		_Btn2 ctrlSetText "Abrir / Cerrar";
...
if(!(_curTarget in life_vehicles)) then {
	_Btn1 ctrlSetText "Comprar Casa";
	...
	if((typeOf _curTarget) in ["Land_i_Garage_V1_F","Land_i_Garage_V2_F"]) then {
		_Btn1 ctrlSetText "Vender Garage";
		...
		if(_curTarget getVariable ["locked",false]) then {
			_Btn2 ctrlSetText "Abrir Armario(s)";
		} else {
			_Btn2 ctrlSetText "Cerrar Armario(s)";
		};
		...
		if(isNull (_curTarget getVariable ["lightSource",ObjNull])) then {
			_Btn3 ctrlSetText "Encender Luces";
		} else {
			_Btn3 ctrlSetText "Apagar Luces";
		};
