...
[] spawn
{
	...
	while {true} do
	{
		...
		if(!([west,getPos player,30] call life_fnc_nearUnits) && (player getVariable["restrained",FALSE]) && vehicle player == player) exitWith {
			...
			detach player;
			titleText["Has sido soltado automaticamente por exceso de tiempo de arresto","PLAIN"];
		};
		
	};
};

titleText[format["Has sido arrestado por %1",_cop getVariable["realname",name _cop]],"PLAIN"];
